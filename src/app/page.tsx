...
# L42-60: Utility functions (cn, safeFetch)
# L62-110: ScraperPage component & State management
# L112-130: Lifecycle and Logging
# L132-150: Search functionality
# L152-220: Scraping Logic (Token Age Check -> Complexes -> Listings)
# L222-250: Data Export (XLSX)
# L252-450: UI Layout (Tailwind CSS)
...
const VERSION = "v3.2.0-cloudtype";
...
export default function ScraperPage() {
  ...
  const startScraping = async () => {
    ...
    try {
      ...
      try {
        ...
        if (ageMin < 60) { token = tokenData.token; addLog(`Using cached token (${Math.round(ageMin)}m old)`); }
        else addLog("Token expired. Fetching fresh one...");
      } catch (e) { addLog("Cache unavailable."); }
      ...
      for (let i = 0; i < complexes.length; i++) {
        ...
        if (complexListings) {
          complexListings.forEach((l: any) => {
            allResults.push({
              complexNo: comp.complexNo, complexName: comp.complexName, address: comp.cortarAddress || selectedRegion.fullName,
              tradeType: l.tradeTypeName, area: `${l.area1}/${l.area2}`, price: l.rentPrc !== 0 ? `${l.dealOrWarrantPrc} / ${l.rentPrc}` : l.dealOrWarrantPrc,
  ...
  const exportExcel = () => {
    ...
    const worksheet = XLSX.utils.json_to_sheet(filtered.map(l => ({ "단지번호": l.complexNo, "아파트명": l.complexName, "주소": l.address, "거래방식": l.tradeType, "면적": l.area, "가격": l.price, "층수": l.floor, "특징": l.feature })));
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="space-y-6">
            ...
            <section className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-4">
              ...
              {searchResults.length > 0 && <div className="max-h-48 overflow-y-auto border border-slate-100 rounded-xl divide-y">{searchResults.map((r) => <button key={r.code} onClick={() => setSelectedRegion(r)} className={cn("w-full text-left px-4 py-3 text-sm transition-colors hover:bg-slate-50", selectedRegion?.code === r.code ? "bg-blue-50 text-blue-700 font-bold" : "text-slate-600")}>{r.fullName}</button>)}</div>}
            </section>
            <section className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-4">
              <div className="p-4 bg-slate-50 rounded-xl border border-dashed border-slate-200 text-center">{selectedRegion ? <div className="space-y-1"><p className="text-xs text-slate-400 font-bold">Selected Region</p><p className="text-sm font-medium text-slate-700">{selectedRegion.fullName}</p></div> : <p className="text-sm text-slate-400">Search and select a region</p>}</div>
              <button disabled={!selectedRegion || isScraping || !backendUrl} onClick={startScraping} className={cn("w-full py-4 rounded-xl flex items-center justify-center gap-2 font-bold transition-all", isScraping ? "bg-slate-100 text-slate-400" : "bg-blue-600 text-white hover:bg-blue-700")}>{isScraping ? <><Loader2 size={20} className="animate-spin" /> Working...</> : "Start Scraping"}</button>
              {isScraping && <div className="w-full bg-slate-100 rounded-full h-2 overflow-hidden"><div className="bg-blue-500 h-full transition-all" style={{ width: `${progress}%` }} /></div>}
            </section>
          </div>
          <div className="lg:col-span-2 space-y-6">
            <section className="bg-white rounded-2xl shadow-sm border border-slate-200 flex flex-col h-[650px]">
              <div className="p-6 border-b flex items-center justify-between">
                <div className="flex items-center gap-4">{["전체", "매매", "전세", "월세"].map(tab => <button key={tab} onClick={() => setActiveTab(tab)} className={cn("text-sm font-semibold pb-1 border-b-2 transition-all", activeTab === tab ? "border-blue-600 text-blue-600" : "border-transparent text-slate-400 hover:text-slate-600")}>{tab}</button>)}</div>
              ...
              <div className="flex-1 overflow-auto relative">
                {listings.length > 0 ? (
                  <table className="w-full text-left text-sm">
                    <thead className="sticky top-0 bg-slate-50 text-slate-500 z-10">
                      <tr><th className="px-4 py-3 border-b">Complex</th><th className="px-4 py-3 border-b">Type</th><th className="px-4 py-3 border-b">Price</th><th className="px-4 py-3 border-b">Area</th></tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50">
                      {listings.filter(l => activeTab === "전체" || l.tradeType === activeTab).map((l, i) => (
                        <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                          ...
                          <td className="px-4 py-3"><span className={cn("px-2 py-0.5 rounded text-[10px] font-bold", l.tradeType === "매매" ? "bg-red-50 text-red-600" : l.tradeType === "전세" ? "bg-blue-50 text-blue-600" : "bg-orange-50 text-orange-600")}>{l.tradeType}</span></td>
                ...
                ) : <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-300"><Info size={48} strokeWidth={1.5} /><p className="mt-4 font-medium">Scraped data will appear here</p></div>}
              </div>
              <div className="bg-slate-900 text-slate-300 p-4 font-mono text-[10px] h-32 overflow-y-auto rounded-b-2xl border-t border-slate-800">
                {logs.map((log, i) => <div key={i} className="mb-1 leading-relaxed border-l border-slate-700 pl-2 ml-1">{log}</div>)}
              </div>
            </section>
          </div>
        </div>
